 ...
 *  
 *  To test sensor readings without uploading to ThinkSpeak comment out 
...
#define ONE_WIRE_BUS 14                           // Digital pin DS18B20 is connected to GIOP5.
#define TEMPERATURE_PRECISION 11                  // Set sensor precision.  Valid options 8,10,11,12 Lower is faster but less precise

unsigned long myChannelNumber = 379430;            // Thingspeak channel ID here
const char * myWriteAPIKey = "3PHTNPOK0GVT1I5A";  // Write API key here
const char* ssid     = "FiOS-4ZNFQ";           // SSID of wireless network
const char* password = "gin823angry7512fad";       // Password for wireless network
...
int updatePeriod = 30;                            //delay in seconds to update to ThingSpeak.  Should be set to not less than 15.
...
void loop() {
    ...
    if(interruptTemp <= 20 && overrideKey != 0){
      coldInterrupt();
    }
